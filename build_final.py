...
function renderUGN(ugn) {
  ...
  const addSetor = (s, linha, rowCls, pctCls, badgeCls) => {
    ...
    rows += `<tr class="setor-row ${rowCls}" onclick="toggleSetor('${key}')">
      ...
      <td>${linha!=='IDENTIDADE'?s.abril:'<span class="zero">—</span>'}</td>
      <td>${linha!=='IDENTIDADE'?s.maio:'<span class="zero">0</span>'}</td>
      <td>${linha!=='IDENTIDADE'?s.junho:'<span class="zero">0</span>'}</td>
      <td>${linha!=='IDENTIDADE'?totS:'<span class="zero">0</span>'}</td>
      <td class="${pctCls}">${linha==='IDENTIDADE'?'<span class="zero">—</span>':pctT}</td>
    </tr>`;
    if (linha === 'IDENTIDADE') {
      rows += `<tr class="detail-row detail-hdr" data-key="${key}"><td></td><td colspan="9" style="color:var(--id);font-size:11px"><i class="ti ti-info-circle"></i> Identidade: cupons distribuídos no C05 · 0 conversões registradas</td></tr>`;
    } else if (s.pdvs && s.pdvs.length) {
      rows += `<tr class="detail-row detail-hdr" data-key="${key}"><td></td><td colspan="2" style="color:var(--ems-blue-mid);font-weight:500;font-size:11px">CNPJ</td><td colspan="4">Nome PDV</td><td colspan="3" style="font-weight:500">Caixas (C04)</td></tr>`;
      s.pdvs.forEach(p => {
