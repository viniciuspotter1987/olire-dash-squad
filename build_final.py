...
h('''
  ...
  <div class="card">
    ...
    <div class="tw"><table id="ugn-table">
...
function renderUGN(ugn) {
  ...
  let html = '';
  let sid = 0;
  const addSetor = (s, linha, cls, pcls, badgeCls) => {
    sid++;
    const key = `s_${ugn.replace(/\s/g,'_')}_${sid}`;
    const c04p = s.c04d ? (s.c04c/s.c04d*100).toFixed(1).replace('.',',')+'%' : '<span class="zero">—</span>';
    const c05p = s.c05d ? (s.c05c/s.c05d*100).toFixed(1).replace('.',',')+'%' : '<span class="zero">—</span>';
    const c04dStr = s.c04d ? fmtBr(s.c04d) : '<span class="zero">—</span>';
    const c05dStr = s.c05d ? fmtBr(s.c05d) : '<span class="zero">—</span>';
    const c04cStr = linha==='IDENTIDADE' ? '<span class="zero">—</span>' : fmtBr(s.c04c);
    const c05cStr = fmtBr(s.c05c);
    html += `<tr class="setor-row ${cls}" onclick="toggleSetor('${key}')">
      ...
      <td>${c04dStr}</td>
      <td>${linha==='IDENTIDADE'?'<span class="zero">—</span>':fmtBr(s.c04c)}</td>
      <td class="${pcls}">${linha==='IDENTIDADE'?'<span class="zero">—</span>':c04p}</td>
      <td>${c05dStr}</td>
      <td>${c05cStr}</td>
      <td class="${pcls}">${linha==='IDENTIDADE'?'<span class="zero">0,0%</span>':c05p}</td>
    </tr>`;
    html += `<tbody id="${key}" class="setor-detail">`;
    if (linha === 'IDENTIDADE') {
      html += `<tr><td></td><td colspan="8" style="color:var(--id);font-size:11px"><i class="ti ti-info-circle"></i> Identidade: cupons distribuídos no C05 · 0 conversões registradas</td></tr>`;
    } else if (s.pdvs && s.pdvs.length) {
      html += `<tr><td></td><td colspan="2" style="color:var(--ems-blue-mid);font-weight:500;font-size:11px">CNPJ</td><td colspan="3">Nome PDV</td><td colspan="3" style="font-weight:500">Caixas C04</td></tr>`;
      s.pdvs.forEach(p => {
        html += `<tr><td></td><td colspan="2">${p.cnpj}</td><td colspan="3">${p.nome}</td><td colspan="3">${fmtBr(p.cx)}</td></tr>`;
      });
    } else {
      html += `<tr><td></td><td colspan="8" style="color:var(--txt3);font-size:11px">Sem dados de PDV disponíveis</td></tr>`;
    }
    html += `</tbody>`;
  };

  // NEXUS block
  html += `<tr class="linha-sep"><td colspan="9"><span class="badge b-nexus">NEXUS</span> — ${u.nexus.length} setores · ${fmtBr(u.nexus_c04conv)} C04 conv. · ${fmtBr(u.nexus_c05conv)} C05 conv.</td></tr>`;
  u.nexus.forEach(s => addSetor(s, 'NEXUS', 'row-nexus', 'pct-n', 'b-nexus'));
  // NEXUS subtotal
  const nxC04d = u.nexus.reduce((a,s)=>a+s.c04d,0), nxC04c = u.nexus_c04conv, nxC05d = u.nexus.reduce((a,s)=>a+s.c05d,0), nxC05c = u.nexus_c05conv;
  html += `<tr class="row-subtotal-n"><td></td><td colspan="2"><strong>Subtotal NEXUS</strong></td><td>${fmtBr(nxC04d)}</td><td>${fmtBr(nxC04c)}</td><td class="pct-n">${nxC04d?(nxC04c/nxC04d*100).toFixed(1).replace('.',',')+"%":"—"}</td><td>${fmtBr(nxC05d)||'—'}</td><td>${fmtBr(nxC05c)}</td><td class="pct-n">—</td></tr>`;

  // VITAL block
  html += `<tr class="linha-sep"><td colspan="9"><span class="badge b-vital">VITAL</span> — ${u.vital.length} setores · ${fmtBr(u.vital_c04conv)} C04 conv. · ${fmtBr(u.vital_c05conv)} C05 conv.</td></tr>`;
  u.vital.forEach(s => addSetor(s, 'VITAL', 'row-vital', 'pct-v', 'b-vital'));
  const vtC04d = u.vital.reduce((a,s)=>a+s.c04d,0), vtC04c = u.vital_c04conv, vtC05d = u.vital.reduce((a,s)=>a+s.c05d,0), vtC05c = u.vital_c05conv;
  html += `<tr class="row-subtotal-v"><td></td><td colspan="2"><strong>Subtotal VITAL</strong></td><td>${fmtBr(vtC04d)}</td><td>${fmtBr(vtC04c)}</td><td class="pct-v">${vtC04d?(vtC04c/vtC04d*100).toFixed(1).replace('.',',')+"%":"—"}</td><td>${fmtBr(vtC05d)||'—'}</td><td>${fmtBr(vtC05c)}</td><td class="pct-v">—</td></tr>`;

  // IDENTIDADE block
  html += `<tr class="linha-sep"><td colspan="9"><span class="badge b-id">IDENTIDADE</span> — ${u.identidade.length} setores · 0 conv. registradas</td></tr>`;
  u.identidade.forEach(s => addSetor(s, 'IDENTIDADE', 'row-id', 'pct-id', 'b-id'));
  const idC05d = u.identidade.reduce((a,s)=>a+s.c05d,0);
  html += `<tr class="row-subtotal-id"><td></td><td colspan="2"><strong>Subtotal IDENTIDADE</strong></td><td class="zero">—</td><td class="zero">—</td><td class="zero">—</td><td>${fmtBr(idC05d)||'—'}</td><td class="zero">0</td><td class="zero">0,0%</td></tr>`;

  // Total UGN
  html += `<tr class="row-total"><td></td><td colspan="2"><strong>TOTAL ${ugn}</strong></td><td>${fmtBr(u.c04dist)}</td><td>${fmtBr(u.c04conv)}</td><td class="pct-e">${(u.c04conv/u.c04dist*100).toFixed(1).replace('.',',')}%</td><td>${fmtBr(u.c05dist)}</td><td>${fmtBr(u.c05conv)}</td><td class="pct-e">${(u.c05conv/u.c05dist*100).toFixed(1).replace('.',',')}%</td></tr>`;

  document.getElementById('ugn-tbody').innerHTML = html;
}
...
function toggleSetor(id) {
  ...
  ico.className = 'ti ' + (open ? 'ti-chevron-down' : 'ti-chevron-right');
...
function renderPDV(ugn) {
  const u = UGN_DATA[ugn];
  if (!u) return;
  // All sectors with PDV data, sorted by c04conv desc
  ...
  all.forEach(s => {
    sid++;
    const key = `pdv_${ugn.replace(/\s/g,'_')}_${sid}`;
    const barW = Math.round(s.c04c/maxC*100);
    html += `<tr class="setor-row" onclick="toggleSetor('${key}')">
      <td><i class="ti ti-chevron-right" id="ico-${key}" style="font-size:12px;color:var(--txt3)"></i></td>
      <td style="font-size:12px">${s.cd} · ${s.nome}</td>
      <td>${fmtBr(s.c04c)}</td>
    ...
    if (s.pdvs && s.pdvs.length) {
      html += `<tr><td></td><td style="color:var(--ems-blue-mid);font-weight:500;font-size:11px">CNPJ</td><td colspan="2">Nome PDV</td><td style="font-weight:500">Cx C04</td></tr>`;
      s.pdvs.forEach(p => {
        html += `<tr><td></td><td style="font-size:11px">${p.cnpj}</td><td colspan="2">${p.nome}</td><td>${p.cx}</td></tr>`;
      });
    } else {
      html += `<tr><td></td><td colspan="4" style="color:var(--txt3);font-size:11px">Sem dados de PDV disponíveis</td></tr>`;
    }
    html += `</tbody>`;
  });
  if (!html) html = `<tr><td colspan="5" style="text-align:center;padding:16px;color:var(--txt3)">Sem conversões registradas nesta UGN</td></tr>`;
  document.getElementById('pdv-tbody').innerHTML = html;
}
